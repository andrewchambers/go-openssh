...
`
	...
	knownHosts, err := ParseKnownHosts([]byte(testFile))
	if err != nil {
		t.Fatal(err)
	}

	if !reflect.DeepEqual(expectedKnownHosts, knownHosts) {
		t.Fatalf("expected %#v != actual %#v", expectedKnownHosts, knownHosts)
	}
